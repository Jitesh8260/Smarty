...
export default function Navbar({ onClearChat, onNewChat, chatHistory, allSessions, onChatClick }: NavbarProps) {
  ...
  useEffect(() => {
    ...
    if (storedTheme) {
      toggleTheme(); // This toggles the theme if needed based on stored value
    }
  ...
  }, []); // Use toggleTheme to ensure theme consistency
  ...
  return (
    <>
      <header className={`p-3 fixed top-0 left-0 w-full z-50 ${theme === 'dark' ? 'bg-slate-900' : 'bg-white shadow-md'} transition-colors duration-300`}>
        <nav className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/" className={`text-2xl font-bold tracking-tight ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
              Smarty
            </Link>
          </div>
...
<div className="md:hidden flex items-center gap-4">
  ...
  >
    {isMenuOpen ? <FaTimes className="text-3xl" /> : <FaBars className="text-3xl" />}
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className={`absolute top-20 right-4 w-48 p-6 rounded-xl shadow-xl ${theme === 'dark' ? 'bg-slate-800' : 'bg-white'} transition-all duration-300`}>
